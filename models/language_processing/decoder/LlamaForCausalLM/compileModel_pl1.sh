...
prec="$2"
num_cores="$3"
prompt_len=$(grep seq_len specializations_pl1.json | head -n1 | grep -Eo '[[:digit:]]+')
ctx_len=$(grep ctx_len specializations_pl1.json | head -n1 | grep -Eo '[[:digit:]]+')
batch_size=$(grep batch_size specializations_pl1.json | head -n1 | grep -Eo '[[:digit:]]+')
...
if [ ! -f "$model_path" ]; then
	model_path="${model_name}/generatedModels/${model_name}_fp16.onnx"
fi
...
/opt/qti-aic/exec/qaic-exec \
	...
	-aic-hw \
	-aic-hw-version=2.0 \
	-network-specialization-config=specializations_pl1.json \
	-convert-to-fp16 \
