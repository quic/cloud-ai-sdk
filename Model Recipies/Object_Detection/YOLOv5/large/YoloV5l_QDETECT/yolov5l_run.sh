...
generate_inputs(){
        ...
        python generate_inputs_onnx_model.py --model-path ./generatedModels/ONNX/yolov5l_416_416_with_abp_qnms.onnx --batch-size 1
}
...
compile_model() {
	if [[ -d ${aic_binary_dir} ]];	then
		rm -ri ${aic_binary_dir}
	        ...
	        compile_options='-aic-hw -aic-hw-version=2.0 -vvv -compile-only -m=./generatedModels/ONNX/yolov5l_416_416_with_abp_qnms.onnx  -aic-binary-dir='${aic_binary_dir}
