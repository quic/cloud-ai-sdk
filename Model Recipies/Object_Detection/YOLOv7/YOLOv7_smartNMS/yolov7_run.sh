...
compile_model() {
	if [[ -d "${aic_binary_dir}" ]]; then
        ...
        compile_options='-aic-hw -aic-hw-version=2.0 -vvv -input-list-file=./input.txt -compile-only -m=./ONNX/yolov7_640_640_smartNMS.onnx  -aic-binary-dir='${aic_binary_dir}
		...
		if [[ "${precision}" == 'fp16' ]]; then
	                compile_options=${compile_options}"  -convert-to-fp16 -aic-num-cores=14"
