...
#       1. Export YOLOvi4 to ONNX model without NMS
...
generate_inputs(){
	...
	python generate_inputs_onnx_model.py --model-path SMARTNMS/yolov4_416_416_without_ABP_NMS_preparator_aic100.onnx --batch-size 1
}

#########################################################################################
# Generate ONNX file of YOLOv4 which is optimized for AIC100 using Model Preparator tool
# Arguments:
#   None
# Outputs:
...
compile_model() {
	...
	compile_options='-aic-hw -aic-hw-version=2.0 -vvv -compile-only -m=SMARTNMS/yolov4_416_416_without_ABP_NMS_preparator_aic100.onnx -aic-binary-dir='${aic_binary_dir}

        if [[ "${config}" -eq 1 ]]; then
                if [[ "${precision}" == 'fp16' ]];  then
                        compile_options=$compile_options" -convert-to-fp16 -batchsize=1 -ols=4 -aic-num-cores=2 multicast-weights  "
                fi

                if [[ "${precision}" == 'int8' ]]; then
                        compile_options=$compile_options" -convert-to-quantize \
                ...
                if  [[ "${precision}" == 'fp16' ]];  then
                        compile_options=${compile_options}"  -convert-to-fp16 -batchsize=1 -ols=1 -aic-num-cores=12 "
                fi
                if [[ "${precision}" == 'int8' ]]; then
