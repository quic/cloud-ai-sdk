...
def generate_inputs(onnx_model_path, batch_size,sequence=128):
    ...
    dims_map = {'src_seq_len':sequence, 'height': 32, 'width':768, 'src_len':sequence,'sequence':sequence,'batch':1,'batch_size':batch_size}
    ...
    in_dir = 'inputFiles/'
    file1 = open(f"input_list_bs{batch_size}_sl{sequence}.txt","w+")
    os.makedirs(in_dir,exist_ok=True)
    for inp in ort_inputs:
        #ort_inputs[inp]=ort_inputs[inp].astype(np.float32)
        ort_inputs[inp].tofile(in_dir+inp+f'_bs{batch_size}_sl{sequence}.raw')
        print(inp, ort_inputs[inp].shape, ort_inputs[inp].dtype)
        file1.write(f"{in_dir}/{inp}_bs{batch_size}_sl{sequence}.raw,")
