...
aic_binary_dir=./Bert-base-cased-onnx-128
...
generate_inputs() {
        ...
        if [[ "${config}" -eq 1 ]]; then
                if [[ "${precision}" == 'mp' ]]; then
                        python generate_inputs_onnx_model.py --model-path ./WORKSPACE/bert-base-cased_simplified_preparator_aic100.onnx --batch-size 8

                elif  [[ "${precision}" == 'fp16' ]]; then
                        python generate_inputs_onnx_model.py --model-path ./WORKSPACE/bert-base-cased_simplified_preparator_aic100.onnx --batch-size 4
                fi


        elif [[ "${config}" -eq 2 ]]; then
                if [[ "${precision}" == 'mp' ]]; then
                        python generate_inputs_onnx_model.py --model-path ./WORKSPACE/bert-base-cased_simplified_preparator_aic100.onnx --batch-size 1
...
compile_model() {
        ...
        compile_options='-aic-hw -aic-hw-version=2.0 -vvv -compile-only -m=./WORKSPACE/bert-base-cased_simplified_preparator_aic100.onnx -onnx-define-symbol=sequence,128 -input-list-file=list.txt -aic-binary-dir='${aic_binary_dir}
...
if [[ -f ./bert-base-cased/generatedModels/bert-base-cased_simplified.onnx  ]]; then
	prepare_model
else
