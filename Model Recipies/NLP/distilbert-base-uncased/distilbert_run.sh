...
#       1. Export distilbert base to ONNX model
...
aic_binary_dir=./distilbert-base-uncased-onnx-128

CONFIG_PATH=distilbert_base.yaml
...
generate_inputs() {
	...
	mkdir inputFiles
        ...
        if [[ "${config}" -eq 1 ]]; then
			...
			python generate_inputs_onnx_model.py --model-path ./WORKSPACE/distilbert-base-uncased_simplified_preparator_aic100.onnx --batch-size 1
...
compile_model() {
        ...
        compile_options='-aic-hw -aic-hw-version=2.0 -vvv -compile-only -m=./WORKSPACE/distilbert-base-uncased_simplified_preparator_aic100.onnx -onnx-define-symbol=seq_length,128 -input-list-file=list.txt -aic-binary-dir='${aic_binary_dir}
...
if [[ -f  ./distilbert-base-uncased/generatedModels/distilbert-base-uncased_simplified.onnx  ]]; then
	prepare_model
else
        err "distilbert Base Model not found. Please make sure distilbert_setup.py is executed"
        exit 1
fi
