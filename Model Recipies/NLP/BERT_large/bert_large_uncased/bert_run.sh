...
aic_binary_dir=./Bert-Large-uncased-onnx-128

CONFIG_PATH=bert_large.yaml
...
generate_inputs() {
	...
	mkdir inputFiles
        ...
        if [[ "${config}" -eq 1 ]]; then
			...
			python generate_inputs_onnx_model.py --model-path ./WORKSPACE/bert-large-uncased_simplified_preparator_aic100.onnx --batch-size 1
...
compile_model() {
        ...
        compile_options='-aic-hw -aic-hw-version=2.0 -vvv -compile-only -m=./WORKSPACE/bert-large-uncased_simplified_preparator_aic100.onnx -onnx-define-symbol=seq_length,128 -input-list-file=list.txt -aic-binary-dir='${aic_binary_dir}
...
if [[ -f ./bert-large-uncased/generatedModels/bert-large-uncased_simplified.onnx  ]]; then
	prepare_model
else
