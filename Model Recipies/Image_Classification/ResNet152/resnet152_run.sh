...
CONFIG_FILE=./resnet152_config.yaml
aic_binary_dir=./resnet152-onnx/
...
generate_inputs(){
        ...
        then
                if [ ${precision} == 'fp16' ]
                then
                        python generate_inputs_onnx_model.py --model-path ./WORKSPACE_RESNET152/ResNet152_preparator_aic100.onnx --batch-size 8
                fi

                if [ ${precision} == 'int8' ]
                then
                        python generate_inputs_onnx_model.py --model-path ./WORKSPACE_RESNET152/ResNet152_preparator_aic100.onnx --batch-size 16
                fi

        elif [ ${config} -eq 2 ]
        then
                        python generate_inputs_onnx_model.py --model-path ./WORKSPACE_RESNET152/ResNet152_preparator_aic100.onnx --batch-size 1
        fi
...
compile_model() {
        ...
        then
                ...
                compile_options='-aic-hw -aic-hw-version=2.0 -vvv -compile-only -m=./WORKSPACE_RESNET152/ResNet152_preparator_aic100.onnx -aic-binary-dir='${aic_binary_dir}
...
###############################################################################
run_model() {
                runner_options=' --time 10  -d 0 -i ./inputFiles/input.1.raw  -t '${aic_binary_dir}
...
if [[ -f "./WORKSPACE_RESNET152/ResNet152_preparator_aic100.onnx" ]]; then
        generate_inputs
        compile_model
else
