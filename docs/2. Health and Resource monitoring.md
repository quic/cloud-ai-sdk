...
- vertical format where all parameters are read once and are listed one parameter per line. 
```
/opt/qti-aic/tools/qaic-util -q 
```
- tabular format where certain parameters are listed in a tabular format, refreshed every 'n' seconds (user input)
```
